...
type Preview = { draft: null | { title?: string; companyName?: string; locations?: string[] }; warnings: string[] };
export function JobForm() {
  ...
  async function loadPreview() { setLoading(true); const response = await fetch("/api/jobs/preview", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ url }) }); setPreview(await response.json() as Preview); setLoading(false); }
